/* 
 * Complete Wardriver for ESP32-S3 with E-Paper Display
 * For LilyGO T5S3-4.7-e-paper-PRO
 * - WiFi and BLE scanning
 * - GPS location tracking
 * - Battery monitoring
 * - SD card logging in Wigle format
 */
...
// SD Card SPI pins - from the working example
#define SD_MISO           21
#define SD_MOSI           13
#define SD_SCK            14
#define SD_CS             12
...
unsigned long sdCardLogInterval = 0; // Counter for SD card logging
String currentLogFilename = "";
String sdCardStatus = "Not Initialized";
int sdLogCount = 0;
...
class MyAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks {
  void onResult(BLEAdvertisedDevice advertisedDevice) {
    ...
    if (sdCardAvailable && gpsValid) {
      // Log immediately to SD card in Wigle format
      String timestamp = String(millis());
      
      String logEntry = address + "," +
                       name + "," +
                       "Unknown," + // AuthMode for BLE
                       timestamp + "," +
                       "0," + // Channel (N/A for BLE)
                       String(rssi) + "," +
                       String(latitude, 6) + "," +
                       String(longitude, 6) + "," +
                       String(altitude, 1) + "," +
                       "0," + // Accuracy - we don't have this value
                       "BLE\n";
                       
      appendToWigleFile(logEntry.c_str());
      sdLogCount++;
    }
...
void initSDCard() {
  // Initialize SD card with the correct pin configuration
  
  // Configure CS pins for all SPI devices to avoid conflicts
  pinMode(SD_CS, OUTPUT);
  digitalWrite(SD_CS, HIGH);
  
  // Check if there's a LORA_CS pin defined and set it high
  if (LORA_CS > 0) {
    pinMode(LORA_CS, OUTPUT);
    digitalWrite(LORA_CS, HIGH);
  }
  
  // Initialize SPI for the SD card with the specified pins
  SPI.begin(SD_SCK, SD_MISO, SD_MOSI);
  
  // Try to mount the SD card
  if (!SD.begin(SD_CS)) {
    Serial.println("SD card initialization failed!");
    sdCardStatus = "Mount Failed";
  ...
  if (!SD.exists("/wigle")) {
    SD.mkdir("/wigle");
    Serial.println("Created /wigle directory");
  }
  ...
  currentLogFilename = "/wigle/wigle_" + timestamp + ".csv";
  
  // Create and write header
  File logFile = SD.open(currentLogFilename, FILE_WRITE);
  if (logFile) {
    // Write Wigle CSV header
    logFile.println("WigleWifi-1.4,appRelease=1.0,model=ESP32-S3,release=1.0,device=LilyGOEPaper,display=true,board=ESP32S3,brand=LilyGO");
    ...
    sdCardStatus = "Ready (" + String(cardSize) + "MB)";
  } else {
    Serial.println("Could not create log file!");
    sdCardStatus = "File Error";
    sdCardAvailable = false;
  }
}

void appendToWigleFile(const char* message) {
...
void setup() {
  ...
  cursor_y += 30;
  char sdInfo[100];
  sprintf(sdInfo, "SD Card: %s", sdCardStatus.c_str());
  epd_write_string(font, sdInfo, &cursor_x, &cursor_y, fb, &font_props);
  
  if (sdCardAvailable) {
    cursor_x = epd_rotated_display_width() / 2; // Reset x position
    cursor_y += 30;
    char fileInfo[100];
    sprintf(fileInfo, "Log: %s", currentLogFilename.c_str());
    epd_write_string(font, fileInfo, &cursor_x, &cursor_y, fb, &font_props);
  }
...
void scanWiFiNetworks() {
  ...
  if (n == 0) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      // Log to SD card if available and we have GPS data
      if (sdCardAvailable && gpsValid) {
        String timestamp = String(millis());
          
        String logEntry = bssid + "," +
                         ssid + "," +
                         encType + "," +
                         timestamp + "," +
                         String(channel) + "," +
...
void updateDisplay() {
  ...
  if (sdCardAvailable) {
    y_pos += 20;
    sprintf(buffer, "SD Log: %d entries", sdLogCount);
    writeTextLine(font, buffer, 15, y_pos, fb);
  }
  
  // GPS Section
  y_pos += 30; // More space before GPS section
...
// Get current date/time components for filename
int year() { return 2025; } // Static year since we don't have RTC
int month() { return 3; }    // Static month
int day() { return 28; }      // Static day
